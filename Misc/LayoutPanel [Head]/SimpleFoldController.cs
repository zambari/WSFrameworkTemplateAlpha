...
public class SimpleFoldController : FoldControlBase
{
    protected LayoutElement layoutElement { get { if (_layoutElement == null) _layoutElement = GetComponent<LayoutElement>(); return _layoutElement; } }
    ...
    public override void SetFold(bool newFold)
    {
        ...
        if (CanStartCoroutine())
            StartCoroutine(FoldRoutine(newFold));
        else
            Debug.Log("cannot start", gameObject);
    ...
    protected virtual IEnumerator FoldRoutine(bool shouldHide)
    {
        if (locked)
        {
        ...
        {
            if (!shouldHide)
                objectsToToggle[currentobj].Show();
            else
                objectsToToggle[currentobj].Hide();
            currentobj++;
            donePerFrame++;

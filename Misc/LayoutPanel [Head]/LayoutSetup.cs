...
namespace Z.LayoutPanel
{
					...
					case Side.Right:
						{
							...
							{
								if (bordersPlacedInside)
									target.offsetMin += new Vector2(0, GetSize(Side.Bottom));
								else
									target.offsetMin += new Vector2(0, -GetSize(Side.Bottom));
								target.offsetMax += new Vector2(0, 0);
							}
							if (sidesFillTopCorners)
							{
								...
								if (bordersPlacedInside)
									target.offsetMax += new Vector2(0, -GetSize(Side.Top));
								else
									target.offsetMax += new Vector2(0, GetSize(Side.Top));
							}
						}
	...
	public class LayoutSetup : ScriptableObject
	{
		...
		[Range(0, 30)]
		public int panelSpacing = 4;
		//	[Range(0, 30)]
		public int borderSize { get { return borderSetup.borderSize; } set { borderSetup.borderSize = value; } }
